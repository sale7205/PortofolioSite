...
import { Phone, Mail, Linkedin, MapPin, Briefcase, Award, Star, Code2, Rocket, Wrench } from "lucide-react"
...
type Domain = "cyan" | "violet" | "amber"

const D: Record<Domain, { tag: string; dot: string; bullet: string; glow: string; active: string; dim: string }> = {
  cyan:   {
    tag:    "text-cyan-300 bg-cyan-500/10 border-cyan-500/25",
    dot:    "bg-cyan-400 ring-cyan-400/20",
    bullet: "bg-cyan-400/70",
    glow:   "border-cyan-400/20 bg-cyan-400/4",
  ...
  violet: {
    ...
    glow:   "border-violet-400/20 bg-violet-400/4",
  ...
  amber:  {
    ...
    glow:   "border-amber-400/20 bg-amber-400/4",
...
const competencies: { label: string; keys: string[]; domain: Domain }[] = [
  { label: "Systems Engineering", keys: ["systems", "prometheus", "cross-functional"], domain: "violet" },
  { label: "GNC",                 keys: ["gnc", "guidance", "control", "pid"],          domain: "cyan"   },
  { label: "EKF / INS",           keys: ["ekf", "kalman", "imu", "navigation"],         domain: "cyan"   },
  { label: "Spacecraft Design",   keys: ["spacecraft", "leo", "satellite", "orbit"],    domain: "violet" },
  { label: "Compressible CFD",    keys: ["cfd", "turbine", "fluent", "aerodynamic"],    domain: "amber"  },
  { label: "Airworthiness",       keys: ["airworthiness", "cmm", "amm", "srm"],         domain: "amber"  },
  { label: "MATLAB",              keys: ["matlab", "ekf", "simulation"],                domain: "cyan"   },
  { label: "ANSYS Fluent",        keys: ["cfd", "fluent", "aerodynamic"],               domain: "amber"  },
  { label: "SolidWorks",          keys: ["solidworks", "turbine", "blade"],             domain: "amber"  },
  { label: "Python",              keys: ["python", "website", "backend"],               domain: "violet" },
  { label: "STK",                 keys: ["stk", "orbit", "satellite"],                  domain: "violet" },
  { label: "CAD / FEA",           keys: ["cad", "solidworks", "structural"],            domain: "amber"  },
]

const experience: { id: string; role: string; company: string; location: string; date: string; domain: Domain; keys: string[]; bullets: string[] }[] = [
  {
    id: "saudia", role: "Aircraft Engineering Intern", company: "Saudia Technic",
    location: "Jeddah, Saudi Arabia", date: "May 2025 – Sept 2025", domain: "amber",
    keys: ["airworthiness", "cmm", "amm", "srm"],
    bullets: [
  ...
  {
    id: "psu", role: "Research Assistant", company: "Penn State University",
    location: "University Park, PA", date: "2021 – Present", domain: "cyan",
...
const projects: { id: string; title: string; sub: string; tag: string; date: string; domain: Domain; keys: string[]; bullets: string[] }[] = [
  {
    id: "prometheus", title: "Prometheus Mission I", sub: "Autonomous On-Orbit Refueling",
    tag: "Systems Eng.", date: "Aug – Dec 2025", domain: "violet",
  ...
  {
    id: "ekf", title: "Extended Kalman Filter", sub: "Lunar Navigation System",
    tag: "GNC", date: "Jan – May 2025", domain: "cyan",
  ...
  {
    id: "drone", title: "Autonomous Drone Landing", sub: "GNC System Design",
    tag: "GNC", date: "Jan – May 2025", domain: "cyan",
  ...
  {
    id: "cfd", title: "Turbine Blade CFD", sub: "Aerodynamic Design & Analysis",
    tag: "Aerodynamics", date: "Aug – Dec 2024", domain: "amber",
...
const leadership: { id: string; role: string; org: string; date: string; domain: Domain; keys: string[]; bullets: string[] }[] = [
  {
    id: "mena", role: "Treasurer", org: "Penn State MENA Caucus Club",
    date: "Dec 2023 – Dec 2024", domain: "violet",
    keys: ["python", "website", "backend"],
    bullets: ["Built and launched a secure finance website for 100+ members using JavaScript, Python & PostgreSQL; managed $10K event budget across 7 fundraisers."],
  },
  {
    id: "consulting", role: "Member", org: "Nittany Lion Consulting Group",
    date: "Jan 2023 – Dec 2025", domain: "violet",
    keys: [],
    bullets: ["Delivered 6+ client engagements; reduced average case turnaround by 20% through standardized hypothesis-driven analysis frameworks."],
  },
]
...
function useTyping(text: string, speed = 65, active = false) {
...
function AnimatedCard({ children, delay = 0, id }: { children: React.ReactNode; delay?: number; id?: string }) {
  const { ref, inView } = useInView()
  return (
    <div id={id} ref={ref} style={{ transitionDelay: `${delay}ms` }}
...
function SectionCard({ icon: Icon, title, children, id }: { icon: React.ElementType; title: string; children: React.ReactNode; id?: string }) {
  ...
  return (
    <div id={id} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      className={`rounded-2xl border bg-card p-6 transition-all duration-300 ${hovered ? "border-white/8 shadow-[0_8px_32px_rgba(0,0,0,0.25)]" : "border-border"}`}>
      <div className="flex items-center gap-3 mb-5">
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border transition-all duration-300 ${hovered ? "bg-primary/15 border-primary/30" : "bg-primary/8 border-primary/15"}`}>
...
function TimelineDot({ domain, active }: { domain: Domain; active: boolean }) {
  return (
    <div className={`shrink-0 w-2.5 h-2.5 rounded-full mt-1.5 ring-4 transition-all duration-300 ${D[domain].dot} ${active ? "scale-125 ring-8" : ""}`} />
  )
}
...
function AnimatedTimeline({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div ref={ref} className="relative">
      <div className="absolute left-[4px] top-3 w-px bg-gradient-to-b from-white/10 to-transparent transition-all duration-1000 ease-out origin-top"
    ...
    <ul className="flex flex-col gap-2">
      {bullets.map((b, i) => (
        <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground leading-relaxed">
          <span className={`w-1.5 h-1.5 rounded-full shrink-0 mt-2 ${D[domain].bullet}`} />
          {b}
        </li>
      ))}
    </ul>
...
function ProjectEntry({ project, highlightedKeys, isLast }: { project: typeof projects[0]; highlightedKeys: string[]; isLast: boolean }) {
  const { ref, inView } = useInView()
  const isHighlighted = highlightedKeys.length > 0 && project.keys.some(k => highlightedKeys.includes(k))
  const d = D[project.domain]
  return (
    <div ref={ref} className={`flex gap-4 transition-all duration-500 ${inView ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"}`}>
      <div className="flex flex-col items-center gap-1 shrink-0">
        <TimelineDot domain={project.domain} active={isHighlighted} />
        {!isLast && <div className="w-px flex-1 bg-border" />}
      </div>
      <div className={`flex-1 pb-4 rounded-xl px-3 py-2 -ml-1 transition-all duration-300 ${isHighlighted ? `border ${d.glow}` : ""}`}>
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-base font-semibold text-foreground">{project.title}</p>
            <span className="text-sm text-muted-foreground/50">— {project.sub}</span>
            <span className={`text-[11px] font-mono px-2 py-0.5 rounded border ${d.tag}`}>{project.tag}</span>
          </div>
          <DatePill>{project.date}</DatePill>
        </div>
        <BulletList bullets={project.bullets} domain={project.domain} />
      </div>
    </div>
  )
}
            ...
            <div className="rounded-2xl border border-border bg-card p-5 flex flex-col gap-5">
              <div className="flex flex-row md:flex-col items-center md:items-start gap-4">
                <div className="relative group shrink-0">
                  <div className="w-20 h-20 md:w-28 md:h-28 rounded-full border-2 border-border bg-secondary overflow-hidden flex items-center justify-center transition-all duration-300 group-hover:border-primary/40 group-hover:shadow-[0_0_20px_rgba(56,189,248,0.1)]">
                    <svg className="w-10 h-10 md:w-14 md:h-14 fill-muted-foreground/30" viewBox="0 0 24 24">
                ...
                <div className="flex-1 md:flex-none">
                  <h1 className="text-xl md:text-[22px] font-bold text-foreground leading-tight tracking-tight">Saleh Aldhafeeri</h1>
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {["CSWE", "STK L3", "MATLAB"].map(b => (
                      <span key={b} className="text-[10px] font-mono px-2 py-0.5 rounded-full border border-border bg-secondary text-muted-foreground">{b}</span>
                    ))}
                  </div>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground font-mono mt-0.5">Jan 2023 – May 2026</p>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-0.5">
                {sections.map(({ id, label }) => (
                  <button key={id} onClick={() => scrollTo(id)}
                    className={`flex items-center gap-3 px-3 py-2 rounded-lg text-left transition-all duration-200 group ${activeSection === id ? "bg-primary/8" : "hover:bg-secondary/40"}`}>
                    <span className={`w-1.5 h-1.5 rounded-full transition-all duration-200 ${activeSection === id ? "bg-primary scale-125" : "bg-border group-hover:bg-muted-foreground/50"}`} />
                    <span className={`text-xs font-medium transition-colors duration-200 ${activeSection === id ? "text-primary" : "text-muted-foreground group-hover:text-foreground"}`}>
                      {label}
                    </span>
                  </button>
                ))}
            ...
            <AnimatedCard id="about" delay={50}>
              <SectionCard icon={Star} title="About" id="about">
                <p className="text-sm leading-relaxed text-muted-foreground">
                  ...
                  experience at Saudia Technic in airworthiness compliance and MRO operations across narrow-body and wide-body fleets.
                </p>
              </SectionCard>
            </AnimatedCard>
                  ...
                  {competencies.map((c) => {
                    ...
                    return (
                      <span key={c.label}
                        ...
                        className={`text-sm px-3 py-1.5 rounded-full border cursor-default transition-all duration-200 select-none ${
                          isActive ? d.active : isDimmed ? `${d.tag} ${d.dim}` : `${d.tag} hover:opacity-80`
                        }`}>
                        {c.label}
                      </span>
                    )
                      ...
                      return (
                        <div key={e.id} className="flex gap-4">
                          <div className="flex flex-col items-center gap-1 shrink-0 z-10">
                            <TimelineDot domain={e.domain} active={isHighlighted} />
                            {i < experience.length - 1 && <div className="w-px flex-1 min-h-4" />}
                          </div>
                          <div className={`flex-1 pb-2 rounded-xl px-3 py-2 -ml-1 transition-all duration-300 ${isHighlighted ? `border ${d.glow}` : ""}`}>
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-lg border border-border bg-secondary/60 flex items-center justify-center shrink-0">
                            ...
                            <div className="ml-11">
                              <BulletList bullets={e.bullets} domain={e.domain} />
                            </div>
                          </div>
                        </div>
                      )
              ...
              <SectionCard icon={Rocket} title="Projects — full docs at SalehAldhafeeri.com" id="projects">
                <AnimatedTimeline>
                  <div className="flex flex-col gap-1 pl-1">
                    {projects.map((p, i) => (
                      <ProjectEntry key={p.id} project={p} highlightedKeys={hoveredSkill} isLast={i === projects.length - 1} />
                    ))}
                  </div>
                </AnimatedTimeline>
              </SectionCard>
              ...
              <SectionCard icon={Award} title="Certifications" id="certs">
                <div className="grid grid-cols-2 gap-2">
                  {certifications.map((c, i) => (
                    <div key={i} className="flex items-center justify-between px-3 py-2.5 rounded-xl border border-border bg-secondary/40 hover:border-primary/20 hover:bg-secondary/70 transition-all duration-200">
                      <span className="text-sm font-medium text-foreground">{c.name}</span>
                      <span className="text-xs font-mono ml-3 shrink-0 text-primary/70">{c.year}</span>
                    </div>
                  ))}
                </div>
              </SectionCard>
                    ...
                    {leadership.map((l, i) => {
                      ...
                      return (
                        <div key={l.id} className="flex gap-4">
                          <div className="flex flex-col items-center gap-1 shrink-0 z-10">
                            <TimelineDot domain={l.domain} active={isHighlighted} />
                            {i < leadership.length - 1 && <div className="w-px flex-1 min-h-4" />}
                          </div>
                          <div className={`flex-1 pb-1 rounded-xl px-3 py-2 -ml-1 transition-all duration-300 ${isHighlighted ? `border ${d.glow}` : ""}`}>
                            <div className="flex items-start justify-between gap-3 mb-1">
                              <div>
                                <p className="text-base font-semibold text-foreground">{l.role}</p>
                                <p className="text-sm text-muted-foreground/70">{l.org}</p>
                              </div>
                              <DatePill>{l.date}</DatePill>
                            </div>
                            <BulletList bullets={l.bullets} domain={l.domain} />
                          </div>
                        </div>
                      )
                    })}
